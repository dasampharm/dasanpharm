        ...
        <div style={{ marginTop: "20px", width: "100%", overflowX: "auto" }}>
          ...
          <div style={{ maxHeight: "400px", overflowY: "auto", position: "relative" }}>
            <table style={{ borderCollapse: "collapse", tableLayout: "auto", width: "100%", fontSize: "14px" }}>
              <thead>
                <tr>
                  <th style={{ padding: "14px", border: "1px solid #999", backgroundColor: "#f7f7f7", color: "#666", textAlign: "left", position: "sticky", top: 0, left: 0, zIndex: 4 }}>제품명</th>
                  {selectedDrug ? null : <th style={{ padding: "14px", border: "1px solid #999", backgroundColor: "#f7f7f7", color: "#666", textAlign: "left", position: "sticky", top: 0, zIndex: 3 }}>성분</th>}
                  {["용량", "제약사", "약가", "요율", "환산액", "품절", "비고"].map((label, i) => (
                    <th key={i} style={{ padding: "14px", border: "1px solid #999", backgroundColor: "#f7f7f7", color: "#666", textAlign: "left", position: "sticky", top: 0, zIndex: 2 }}>{label}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {getFilteredDrugs().map((drug, index) => (
                  <tr key={index}>
                    <td style={{ padding: "14px", border: "1px solid #999", background: "#fff", color: "#666", wordBreak: "break-word", overflowWrap: "anywhere", whiteSpace: drug["제품명"]?.length >= 9 ? "normal" : "nowrap", position: "sticky", left: 0, zIndex: 1 }}>{drug["제품명"]}</td>
                    {selectedDrug ? null : <td style={{ padding: "14px", border: "1px solid #999", wordBreak: "break-word", overflowWrap: "anywhere", whiteSpace: "normal" }}>{drug["성분"]}</td>}
                    {["용량", "제약사", "약가", "요율", "환산액", "품절", "비고"].map((key, i) => (
                      <td key={i} style={{ padding: "14px", border: "1px solid #999", whiteSpace: key === "비고" ? "nowrap" : "normal", overflow: key === "비고" ? "visible" : "hidden", wordBreak: key === "비고" ? "normal" : "break-word", overflowWrap: "anywhere" }}>{drug[key]}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
